    ...
    <div class="relative bg-white/80 backdrop-blur-sm rounded-2xl p-8">
      ...
      <div class="flex flex-col md:flex-row items-center justify-center gap-8 mb-8">
        <div class="w-36 h-36 flex-shrink-0 rounded-full bg-gradient-to-br from-[#F5F0FF] to-[#EBE3FF] flex items-center justify-center relative shadow-lg">
          ...
          <span class="text-5xl font-bold text-[#6838F0]">{{ cri.toFixed(1) }}</span>
        </div>
        ...
        <div class="bg-[#F8F7FF] rounded-xl p-6 relative">
          ...
          <div class="flex flex-col items-center max-w-3xl mx-auto">
            ...
            <div class="w-full bg-white rounded-lg p-4 shadow-sm mb-2 flex justify-between items-center">
              ...
              <span class="text-xl font-bold text-[#6838F0]">{{ sii.toFixed(1) }}</span>
            </div>
            
            <!-- 分数线 -->
            <div class="w-full h-1 bg-[#6838F0] my-2 rounded-full"></div>
            ...
            <div class="w-full bg-white rounded-lg p-4 shadow-sm mt-2 mb-4 flex justify-between items-center">
              <div class="flex items-center">
                ...
                <div>
                  <h5 class="font-semibold text-gray-800">安全能力指数</h5>
                  <p class="text-xs text-gray-500">量化企业安全防护能力和覆盖范围</p>
                </div>
              </div>
              <span class="text-xl font-bold text-[#6838F0]">{{ sai.toFixed(1) }}</span>
            </div>
            ...
            <div class="flex items-center mt-4">
              ...
              <div class="flex items-center bg-white rounded-lg px-6 py-3 shadow-sm">
                ...
                <span class="text-xl font-bold text-[#6838F0]">{{ cri.toFixed(1) }}</span>
              </div>
            </div>
          </div>
          
          <div class="mt-6 text-center text-sm text-gray-600 max-w-2xl mx-auto">
            <p>CRI = SII ÷ SAI：综合风险指数是安全问题指数除以安全能力指数得出的结果。SII 越高或 SAI 越低，风险指数就越高。</p>
            <p class="mt-2">这种计算方式直观地反映了企业安全状况：<strong>问题越多，能力越弱，风险越大</strong></p>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        >
          <div class="flex items-start">
            ...
            <div>
              ...
              <p class="text-lg font-semibold text-[#6838F0]">{{ sii.toFixed(1) }}</p>
            </div>
          </div>
        ...
        >
          <div class="flex items-start">
            ...
            <div>
              <h4 class="font-semibold text-gray-800">安全能力指数</h4>
              <p class="text-xs text-gray-500 mb-1">Security Ability Index</p>
              <p class="text-lg font-semibold text-[#6838F0]">{{ sai.toFixed(1) }}</p>
            </div>
          </div>
          <p class="text-sm text-gray-600 mt-3">反映企业部署的安全能力的强度、覆盖范围</p>
        </div>
      </div>
      ...
      <div v-if="showSiiDetails" class="mt-8 bg-white/90 rounded-xl p-8 shadow-sm border-l-4 border-[#6838F0] border-t border-r border-b border-purple-100 animate-slideIn">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div class="bg-[#F8F7FF] rounded-xl p-6">
            <div class="flex items-center mb-4">
              <i class="i-carbon-warning-alt text-[#6838F0] text-2xl mr-3"></i>
              <h5 class="text-lg font-semibold">问题等级权重</h5>
            </div>
            <p class="text-gray-600 text-sm">严重、高危问题占比越高，SII 越高</p>
            <div class="mt-4 flex justify-between items-end w-full gap-4">
              <div class="flex flex-col items-center">
                <div class="h-16 w-10 rounded-t-md bg-red-500"></div>
                <span class="text-xs text-gray-600 mt-2">严重</span>
              </div>
              <div class="flex flex-col items-center">
                <div class="h-12 w-10 rounded-t-md bg-orange-400"></div>
                <span class="text-xs text-gray-600 mt-2">高危</span>
              </div>
              <div class="flex flex-col items-center">
                <div class="h-8 w-10 rounded-t-md bg-yellow-400"></div>
                <span class="text-xs text-gray-600 mt-2">中危</span>
              </div>
              <div class="flex flex-col items-center">
                <div class="h-5 w-10 rounded-t-md bg-blue-400"></div>
          ...
          <div class="bg-[#F8F7FF] rounded-xl p-6">
            ...
            <p class="text-gray-600 text-sm">发现的安全问题总数越多，SII 越高</p>
            <div class="mt-4 relative h-20">
              <div class="absolute bottom-0 left-0 w-full h-full flex items-end">
                <div class="relative w-full bg-[#EBE3FF] h-16 rounded">
          ...
          <div class="bg-[#F8F7FF] rounded-xl p-6">
            ...
            <p class="text-gray-600 text-sm">问题数量一致的情况下，应用数量越少，SII 越高</p>
            <div class="mt-4 relative">
              ...
              <div class="flex gap-2">
                <div class="flex-1 bg-red-500/20 h-24 rounded flex items-center justify-center relative">
                ...
                <div class="flex-1 bg-blue-400/20 h-24 rounded flex items-center justify-center relative">
              ...
              <p class="text-right text-xs text-gray-600 mt-2">问题密度比较: 4:1</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="showSaiDetails" class="mt-8 bg-white/90 rounded-xl p-8 shadow-sm border-l-4 border-[#6838F0] border-t border-r border-b border-purple-100 animate-slideIn">
        <div class="flex items-center mb-6">
          ...
          <div>
            <h4 class="text-xl font-semibold text-gray-800">安全能力指数 (SAI) 的构成因素</h4>
            <p class="text-sm text-gray-600">了解影响安全能力指数的关键因素和维度</p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="bg-[#F8F7FF] rounded-xl p-6">
            ...
            <p class="text-gray-600 text-sm">重要安全能力（如 SAST, SCA）的覆盖对 SAI 贡献更大</p>
            <div class="mt-4 space-y-2">
              <div class="flex items-center gap-2">
                <span class="text-sm min-w-16">SAST</span>
                <div class="w-full bg-[#EBE3FF] h-3 rounded-full">
                  <div class="bg-[#6838F0] h-3 rounded-full" style="width: 90%"></div>
                </div>
                <span class="text-xs min-w-8 text-right">9.0</span>
              </div>
              <div class="flex items-center gap-2">
                <span class="text-sm min-w-16">SCA</span>
                <div class="w-full bg-[#EBE3FF] h-3 rounded-full">
                  <div class="bg-[#6838F0] h-3 rounded-full" style="width: 85%"></div>
                </div>
                <span class="text-xs min-w-8 text-right">8.5</span>
              </div>
              <div class="flex items-center gap-2">
                <span class="text-sm min-w-16">DAST</span>
                <div class="w-full bg-[#EBE3FF] h-3 rounded-full">
                  <div class="bg-[#6838F0] h-3 rounded-full" style="width: 70%"></div>
                </div>
                <span class="text-xs min-w-8 text-right">7.0</span>
              </div>
              <div class="flex items-center gap-2">
                <span class="text-sm min-w-16">容器安全</span>
                <div class="w-full bg-[#EBE3FF] h-3 rounded-full">
                  <div class="bg-[#6838F0] h-3 rounded-full" style="width: 60%"></div>
                </div>
                <span class="text-xs min-w-8 text-right">6.0</span>
              </div>
            </div>
          </div>
          ...
          <div class="bg-[#F8F7FF] rounded-xl p-6">
            ...
            <p class="text-gray-600 text-sm">安全能力覆盖的应用数量越多，SAI 越高</p>
            <div class="mt-6 flex flex-col items-center justify-center">
              <div class="w-32 h-32 rounded-full border-8 border-[#EBE3FF] flex items-center justify-center relative overflow-hidden">
                <div class="absolute inset-0 pie-chart">
                  <div class="w-full h-full rounded-full bg-[#6838F0]"></div>
                </div>
              </div>
              <p class="text-right text-xs text-gray-600 mt-4 w-full">应用覆盖率: 75%</p>
            </div>
          </div>
        </div>
      </div>
...
const sii = ref(87.5)  // Security Issue Index
const sai = ref(3.1)  // Security Ability Index
const cri = computed(() => sii.value / sai.value)  // Comprehensive Risk Index
